...
import { numberOfBrush } from "./scatterPlot.js";
let scaleColor;
...
function selectCircles(event) {
  ...
  if (mode == "Visualize") {
    ...
    if (event.target.classList.length > 0) {
      ...
      var addingSet = allData.filter(function (obj) {
        return obj.Category == event.target.id;
      });
      if (oldSet == undefined || oldSet.length == 0) {
    ...
    if (numberOfCheckBoxSelected > 1) {
      ...
      d3.select("#scatterPlot").selectAll("circle").classed("group1", false);
       d3.select("#scatterPlot").selectAll("circle").classed("group2", false);
    }
    ...
    if (event.target.classList.length > 0) {
      ...
      if (commonService.isEmpty(commonService.firstSet)) {
        ...
        selectedCategory.forEach((circle) => {
          circle.classList.add("selectedScatterPlot");
          circle.classList.add("group1");
        });
