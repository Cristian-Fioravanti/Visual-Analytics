...
export const customColors = [
    ...
    "#FFFF00",
    "#00FF00",
    "#FF0000", // Esempi di colori aggiuntivi
    "#FFD700",
    "#9400D3",
    "#8B008B",
];
...
export function initializeScaleColor(jsonPCAData) {
    ...
    scaleColor = d3
        .scaleOrdinal()
        .domain(distinctPCAData["Category"])
        .range(customColors);
}
...
export function deleteFilters(key, value) {
    filters.value.delete(key, value);
}
...
export function disabledCheckBox() {
    let nodeListCheckbox = Array.from($("input:checkbox")).filter(
        (checkbox) => checkbox.classList.length == 0
    );
