...
import * as parallelService from "./parallelCoordinates.js";

let dimensionsString = ["Content_Rating", "Type"];

export function populateBoxplots(data) {
...
function createBoxPlot(data, i, title) {
    ...
    function countFilterElimintation(title, rect) {
        ...
        let sizeAttuale = Array.from(d3.select("#scatterPlot").selectAll("circle.selectedScatterPlotFiltered")._groups[0]).length;
        if (Array.from(rect.classList).indexOf("selectedBoxPlotRect") != -1) {
            deleteTitleSection(rect, title);
            addBorderToCircleSelected(true);

            listDataSelected = Array.from(d3.select("#scatterPlot").selectAll("circle.selectedScatterPlotFilteredTmp")._groups[0]);

            number = listDataSelected.length - sizeAttuale;
            setNewSections(rect, title);
            d3.select("#scatterPlot").selectAll("circle.selectedScatterPlotFilteredTmp").classed("selectedScatterPlotFilteredTmp",false)
        } else {
            setNewSections(rect, title);
            addBorderToCircleSelected(true);
    ...
    var clickRect = function (d) {
        ...
        addBorderToCircleSelected(false);
    };
    ...
    function setNewSections(rect, title) {
        ...
        let titleValue;
        titleValue = commonService.getFilters(title);
        if (titleValue != undefined) {
            titleValue.push([rangeMin, rangeMax]);
        } else {
            titleValue = [[rangeMin, rangeMax]];
        }
        commonService.setFilters(title, titleValue);
    }
    ...
    function deleteTitleSection(rect, title) {
        ...
        for (let i = 0; i < titleValue.length; i++) {
            ...
            if (element[0] == rangeMin && element[1] == rangeMax) {
                ...
                if (titleValue.length == 0) commonService.deleteFilters(title);
                break;
            }
        }
    }
...
function addBorderToCircleSelected(tmp) {
    d3.select("#scatterPlot")
        .selectAll("circle.selectedScatterPlot")
        .each(function (d) {
            let isActive = commonService.filters.value.size > 0 ? 
                Array.from(commonService.filters.value).every(([title, listRange]) => {
                    if (dimensionsString.includes(title)) {
                        let ris = true
                        let y = parallelService.getY()
                        for (let i = 0; i < listRange.length; i++) {
                            const element = listRange[i];
                            let rangeMin = element[0];
                            let rangeMax = element[1];
                            const yCoordinate = y[title](d[title]);
                            ris = ris &&  yCoordinate >= rangeMin && yCoordinate <= rangeMax;
                        }
                        return ris;
                    } else {
                        let ris = true;
                ...
                : false;
            if (isActive)
                if(tmp) d3.select(this).classed("selectedScatterPlotFilteredTmp", true);
                else d3.select(this).classed("selectedScatterPlotFiltered", true);
            else 
                if(tmp) d3.select(this).classed("selectedScatterPlotFilteredTmp", false);
                else d3.select(this).classed("selectedScatterPlotFiltered", false);
    });
}